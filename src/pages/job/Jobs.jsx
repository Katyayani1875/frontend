...
const SearchBar = ({ onSearch, initialSearch }) => {
    ...
    return (
        <motion.div
            ...
            className="w-full max-w-3xl mx-auto rounded-2xl p-2.5 shadow-xl border border-slate-200 dark:border-slate-700 flex items-center gap-2 relative z-10 frosted-glass"
        >
            <div className="flex-1 relative">
                <Search size={20} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400" />
                <input
                    type="text"
                    placeholder="Search job titles, skills, or keywords..."
                    className="w-full py-2.5 pl-10 pr-3 rounded-xl bg-transparent text-slate-800 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-base"
                    value={searchTerm}
            ...
            <Button
                ...
                className="w-auto h-auto py-2.5 px-6 text-base font-semibold bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 transition-all rounded-xl shadow-md flex items-center group"
            >
                <span className="opacity-100 group-hover:opacity-0 transition-opacity duration-200">Search</span>
                <Search size={18} className="absolute opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
            </Button>
            {searchTerm && (
                <Button
                    onClick={clearSearch}
                    variant="ghost"
                    size="icon"
                    className="absolute top-1.5 right-1.5 md:relative md:top-auto md:right-auto text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 transition-colors"
                >
                    <X size={18} />
                </Button>
            )}
...
// Job List Item Component (Enhanced and Scaled)
  ...
  return (
    ...
    >
      <Link to={`/jobs/${job._id}`} className="block">
        <div
          className={`p-5 rounded-2xl border-2 transition-all duration-300 ease-in-out
            ${isActive
        ...
        >
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-snug mb-2">
            {job.title}
          </h3>
          <div className="flex flex-wrap items-center text-slate-500 dark:text-slate-400 text-xs gap-x-4 gap-y-1 flex-grow">
          ...
          {isActive && (
              <motion.div layoutId="active-job-indicator" className="absolute left-0 top-0 bottom-0 w-1.5 bg-gradient-to-b from-indigo-600 to-purple-600 rounded-l-2xl" />
          )}
        </div>
      </Link>
    </motion.div>
...
// Featured Job Display Component (Enhanced and Scaled)
  ...
  if (!job) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-white dark:bg-slate-800 rounded-3xl border-2 border-dashed border-slate-300 dark:border-slate-700 shadow-xl">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-xs text-sm">Hover over a job on the left to see its details here.</p>
        <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-xs text-sm">Or try a different search or category.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 h-full flex flex-col">
      <div className="flex-grow">
        ...
        <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mt-2 leading-tight">
            {job.title}
        </h2>
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500 dark:text-slate-400 text-sm mt-4">
          ...
          {job.salaryRange && (
             <div className="flex items-center gap-1.5"><DollarSign size={16} />{job.salaryRange}</div>
          )}
        </div>
        <p className="text-base text-slate-600 dark:text-slate-300 mt-6 leading-relaxed line-clamp-[9] md:line-clamp-[12] min-h-[10rem] overflow-hidden">
            {job.description}
        </p>
      ...
      <div className="mt-auto pt-8">
        <Link to={`/jobs/${job._id}`}>
          <Button size="lg" className="w-full font-bold text-base py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl shadow-lg hover:shadow-xl transition-all">
            View Full Details & Apply <ChevronsRight size={18} className="ml-2" />
          </Button>
        </Link>
      </div>
    </div>
...
// Skeleton Loader for FeaturedJobDisplay (Scaled to match new sizes)
const FeaturedJobDisplaySkeleton = () => (
    <div className="p-8 bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 h-full flex flex-col animate-pulse">
        ...
        <div className="h-8 bg-slate-200 dark:bg-slate-700 rounded w-3/4 mt-2 mb-3"></div>
        <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4">
            <div className="h-4 bg-slate-200 dark:bg-slate-700 rounded w-1/4"></div>
            <div className="h-4 bg-slate-200 dark:bg-slate-700 rounded w-1/4"></div>
        </div>
        <div className="mt-6 space-y-2 flex-grow">
            <div className="h-3.5 bg-slate-200 dark:bg-slate-700 rounded"></div>
            <div className="h-3.5 bg-slate-200 dark:bg-slate-700 rounded w-11/12"></div>
            ...
            <div className="h-3.5 bg-slate-200 dark:bg-slate-700 rounded w-10/12"></div>
        </div>
        <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
            <div className="h-4 bg-slate-200 dark:bg-slate-700 rounded w-1/3 mb-3"></div>
            <div className="flex flex-wrap gap-2">
                {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-7 w-18 bg-slate-200 dark:bg-slate-700 rounded-full"></div>
                ))}
            </div>
        </div>
        <div className="mt-auto pt-8">
...
const Jobs = () => {
  ...
  // Modify fetchJobs to handle search and category
  ...
  const handleSearchBarSubmit = (searchVal) => {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 to-white dark:from-slate-950 dark:to-gray-900 min-h-screen relative overflow-hidden">
        {/* Subtle background gradients/blobs for aesthetic */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-96 bg-purple-300 dark:bg-purple-800 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob"></div>
        <div className="absolute bottom-0 right-0 w-80 h-80 bg-indigo-300 dark:bg-indigo-800 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob animation-delay-2000"></div>
        ...
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-24 relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-16">
                ...
                <p className="mt-4 text-lg text-slate-700 dark:text-slate-300 max-w-3xl mx-auto font-normal leading-relaxed">
                   {selectedCategorySlug
            ...
            >
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-5">Explore by Category</h3>
                {loadingCategories ? (
                    <div className="flex justify-center flex-wrap gap-2">
                        {Array.from({ length: 7 }).map((_, i) => (
                            <div key={i} className="h-9 w-28 bg-slate-200/80 dark:bg-slate-800/50 rounded-full animate-pulse"></div>
                        ))}
                    </div>
                ) : (
                    <div className="flex flex-wrap justify-center gap-2">
                        <Button
                            ...
                            variant={!selectedCategorySlug && !searchTerm ? "default" : "outline"}
                            className={`${(!selectedCategorySlug && !searchTerm) ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border-slate-300 dark:border-slate-700'} rounded-full px-4 py-1.5 text-sm font-semibold transition-all duration-200`}
                        >
                            All Categories
                        </Button>
                        {categories.map((category) => (
                            <Button
                                ...
                                className={`${selectedCategorySlug === category.slug ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border-slate-300 dark:border-slate-700'} rounded-full px-4 py-1.5 text-sm font-semibold transition-all duration-200`}
                            >
                                {category.name}
                            </Button>
                        ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start mt-12">
                ...
                <div className="space-y-3">
                    {loading ? (
                        Array.from({ length: 7 }).map((_, i) => (
                            <div key={i} className="h-[120px] bg-slate-200/80 dark:bg-slate-800/50 rounded-xl animate-pulse shadow-md"></div>
                        ))
                    ) : (
                        <AnimatePresence>
                            {jobs.length > 0 ? (
                                ...
                                <motion.div
                                    ...
                                    exit={{ opacity: 0 }}
                                    transition={{ duration: 0.3 }}
                                    className="text-center py-12 text-slate-500 bg-white dark:bg-slate-800/40 rounded-xl shadow-md border border-slate-200 dark:border-slate-700"
                                >
                                    ...
                                    {(searchTerm || selectedCategorySlug) && (
                                        <Button
                                            onClick={clearAllFilters}
                                            variant="outline"
                                            className="mt-5 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-700 rounded-full text-sm"
                                        >
                                            Reset Filters
                                        </Button>
                                    )}
                    ...
                    {!loading && totalPages > 1 && (
                        <motion.div
                            ...
                            className="flex justify-center items-center pt-6 space-x-2"
                        >
                            <Button
                                onClick={() => setPage(p => Math.max(1, p - 1))}
                                disabled={page === 1}
                                variant="ghost"
                                className="text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 px-4 py-2 text-sm"
                            >
                                Previous
                            </Button>
                            <span className="font-medium text-slate-700 dark:text-slate-300 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-md text-sm">
