...
import { ArrowRight, Search, Briefcase, Bot, Users, Cpu, MessageSquare, FileText, Star } from "lucide-react";
import { toast } from "react-hot-toast";
import { Link, useNavigate } from "react-router-dom"; // <-- ADD useNavigate

// --- Framer Motion Animation Variants (can be shared) ---
...
const HeroSection = ({ onSearch }) => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (!jobTitle.trim()) {
      toast.error("Please enter a job title, skill, or company.");
      return;
    }
  ...
  return (
    <motion.section 
      className="relative flex flex-col justify-center min-h-[calc(100vh-8rem)] text-center overflow-hidden bg-slate-50 dark:bg-slate-900"
    ...
    >
      ...
      <div className="flex-grow flex flex-col items-center justify-center relative z-10 px-4 pt-16">
      ...
      <motion.div className="w-full pb-12 px-4" variants={{hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 100, damping: 20 }}}}>
          ...
          <div className="flex items-center">
            <div className="flex -space-x-2 mr-2">
              {['men/32', 'women/44', 'men/45'].map((id, i) => (
                <img key={i} src={`https://randomuser.me/api/portraits/thumb/${id}.jpg`} alt="User" className="w-7 h-7 rounded-full border-2 border-slate-50 dark:border-slate-900"/>
              ))}
            </div>
      ...
      <div className="w-full overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_10%,_black_90%,transparent_100%)]">
        <Marquee gradient={false} speed={40} pauseOnHover>
          {duplicatedLogos.map((logo, index) => (
            <img key={`${logo.name}-${index}`} src={logo.src} alt={logo.name} className="h-8 md:h-9 object-contain mx-10 opacity-70 hover:opacity-100 transform hover:scale-110 transition-all duration-300"/>
          ))}
        </Marquee>
      </div>
    </div>
...
// ---SECTION 3: WHY JOBHUNT (VALUE PROPS)---
const WhyJobHuntSection = () => {
    const features = [
    { icon: <Cpu size={24} className="text-indigo-300" />, title: "AI-Powered Matching", description: "Our intelligent algorithms go beyond keywords to match your unique skills with the perfect role." },
    { icon: <Bot size={24} className="text-purple-300" />, title: "Career Co-pilot", description: "From resume analysis to interview prep, get personalized AI assistance at every step." },
    { icon: <Users size={24} className="text-sky-300" />, title: "Direct to Recruiters", description: "Connect with decision-makers at top companies actively searching for talent like you." }
  ];
  ...
  return (
    <section className="relative py-24 sm:py-32 bg-slate-50 dark:bg-slate-900 overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-grid-slate-900/[0.04] bg-[bottom_1px_center] dark:bg-grid-slate-400/[0.05] dark:bg-bottom mask-gradient"></div>
        <motion.div className="absolute inset-x-0 top-[-200px] h-[500px] bg-gradient-to-tr from-purple-500/20 via-indigo-500/20 to-sky-500/20 blur-[150px]" initial={{ opacity: 0, y: 100 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.8 }}/>
      </div>
      <div className="container mx-auto px-4 relative z-10">
        <motion.div className="text-center max-w-3xl mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.5 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-slate-900 dark:text-white">Don't Just Find a Job. Build a Career.</h2>
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-400">JobHunt is more than a job board. We're your strategic partner in career growth.</p>
        </motion.div>
        <motion.div className="mt-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={{ visible: { transition: { staggerChildren: 0.15 } } }}>
          {features.map((feature, i) => ( <FeatureCard feature={feature} key={i} /> ))}
        </motion.div>
      </div>
    </section>
  );
...
const FeatureCard = ({ feature }) => { /* ... Your existing FeatureCard code from previous steps ... */ };
const FeatureShowcase = () => { /* ... Your existing FeatureShowcase code from previous steps ... */ };
const HowItWorksSection = () => { /* ... Your existing HowItWorksSection code from previous steps ... */ };

// --- THE MAIN HOME PAGE COMPONENT ---
export default function Home() {
  const navigate = useNavigate(); // <-- Get the navigate function

  // THIS IS THE NEW, CORRECT handleSearch function.
  // Its only job is to redirect to the /jobs page.
  ...
  return (
    <div className="bg-white dark:bg-[#070912] text-slate-800 dark:text-slate-200">
      {/* The Hero Section now handles its own layout and passes the search term up */}
      <HeroSection onSearch={handleSearch} />
      
      {/* All other sections go inside the main container for consistent padding */}
